...
/**
 * This is part of https://github.com/LarsHagemann/bimap/blob/master/bimap.hpp from
 * LarsHagemann/bimap
 *
 * Obeying MIT License.
 *
...
namespace luna::details::bimap {
	...
	public:
		...
		public:
			...
			typedef pair<typename _Key_Container::iterator, typename _Mapped_Container::iterator>
						value_type;
	...
	public:
		...
		Bimap(std::initializer_list<_Tree_Node> init) {
			for (const auto& elem : init) insert(elem);
		}
	...
	public:
		/* Helper functions */
		bool has_key(const key_type& key) const { return m_key_tree.find(key) != m_key_tree.end(); }
	...
	using UnorderedBimap =
		Bimap<_KeyType, _ValueType, std::unordered_set<_KeyType>, std::unordered_set<_ValueType>>;
}  // namespace luna::details::bimap

namespace luna {
	using details::bimap::Bimap;
	using details::bimap::UnorderedBimap;
}  // namespace luna